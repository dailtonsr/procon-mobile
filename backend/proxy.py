#!/usr/bin/env python3
"""
PRoCon Mobile — WebSocket ↔ RCON Proxy
Deploy: Railway.app
"""
...
async def handler(ws):
    ...
    async def refresh_info():
        ...
        if r and r[0]=='OK' and len(r)>5:
            scores = {}
            try:
                # BF4 serverInfo: scores at index 8,9
                scores = {'team1': int(r[8]) if len(r)>8 else 0,
        ...
        async for raw in ws:
            ...
            if msg['type'] == 'connect':
                ...
                try:
                    ...
                    print(f"[rcon] TCP → {host}:{port}")
                except Exception as e:
                    await ws.send(json.dumps({'type':'error','message':f'Cannot connect to {host}:{port}: {e}'}))
                    continue
                ...
                if not r1 or r1[0]!='OK' or len(r1)<2:
                    await ws.send(json.dumps({'type':'error','message':'RCON handshake failed'}))
                    continue

                hashed = hashlib.md5(
                    bytes.fromhex(r1[1]) + passwd.encode('utf-8')
                ).hexdigest().upper()

                r2 = await rcon('login.hashed', hashed)
                if not r2 or r2[0]!='OK':
                    await ws.send(json.dumps({'type':'error','message':'Wrong RCON password'}))
                    continue
            ...
            # COMMAND
                ...
                if not writer:
                    await ws.send(json.dumps({'type':'error','message':'Not connected to RCON'}))
                    continue
                cmd  = msg.get('cmd','')
                args = msg.get('args',[])
...
# ── MAIN ───────────────────────────────────────────────────
async def main():
    print(f"PRoCon Proxy — port {PORT}")
